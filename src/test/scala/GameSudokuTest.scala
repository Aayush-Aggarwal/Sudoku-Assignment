...
/**
  * Created by knoldus on 7/25/17.
  */
class GameSudokuTest extends FunSuite{
  ...
  val sudoObj = new GameSudoku

  test("Testing for sudoku where solution exists")
  {
    assert(sudoObj.puzzleSolver(grid))
  }

  test("Testing for sudoku with all 0s")
  {
    assert(sudoObj.puzzleSolver(gridWithZeros))
  }

  test("Testing for sudoku where solution doesn't exist")
  {
    assert(sudoObj.puzzleSolver(gridFail) === false)
  }
}
