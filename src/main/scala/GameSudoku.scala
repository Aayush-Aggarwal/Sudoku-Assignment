...
class GameSudoku {
  ...
  def puzzleSolver(grid: Vector[Vector[Int]]): Boolean = {
    ...
    def solver(grid: Vector[Vector[Int]], row:Int, col:Int, prevVal:Int = 0): Boolean = {
      if (row == 9) {println(grid.map(_.mkString(" ")).mkString("\n") + "\n")
        ...
        val (row,col) = findUnassignedPosition(grid)
        if(row == -1 && col == -1)  {true}
        else {
  ...
  private def findUnassignedPosition(grid: Vector[Vector[Int]]): (Int,Int) = {
